...
function ensureUpdateBar() {
  ...
  bar.innerHTML = '<div id="autoUpdateText" style="color:#fff;font-size:12px;font-family:"DM Sans",sans-serif;white-space:nowrap;opacity:0;transition:opacity 0.2s;"></div>';
...
function showProgress(show, percent = 0, text = '') {
  ...
  if (show && percent > 0) {
    ...
    bar.innerHTML = `<div style="position:absolute;left:0;top:0;bottom:0;background:rgba(255,255,255,0.25);width:${percent}%;transition:width 0.3s;"></div><div id="autoUpdateText" style="color:#fff;font-size:12px;font-family:"DM Sans",sans-serif;white-space:nowrap;position:relative;z-index:1;display:flex;align-items:center;gap:8px;"><span>${text}</span><span style="opacity:0.8;">${percent}%</span></div>`;
  } else if (show) {
    bar.style.height = '36px';
    bar.innerHTML = `<div id="autoUpdateText" style="color:#fff;font-size:12px;font-family:"DM Sans",sans-serif;white-space:nowrap;position:relative;z-index:1;">${text}</div>`;
  } else {
...
function showUpdateComplete(version, releaseNotes) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--bg-card);border-radius:16px;padding:32px;max-width:420px;width:90%;box-shadow:var(--shadow-lg);text-align:center;">
      ...
      <h3 style="font-family:'Crimson Pro',serif;font-size:20px;color:var(--text-primary);margin-bottom:8px;">${t('update_ready_title') || 'Update Ready'}</h3>
      ...
      <div style="display:flex;gap:12px;justify-content:center;margin-top:20px;">
        <button onclick="dismissUpdateModal()" style="padding:8px 20px;border:1px solid var(--border-color);border-radius:8px;background:transparent;color:var(--text-secondary);cursor:pointer;font-size:13px;font-family:'DM Sans',sans-serif;">
          ${t('update_later') || 'Later'}
        </button>
        <button onclick="installAndRestart()" style="padding:8px 24px;border:none;border-radius:8px;background:linear-gradient(135deg,#0077e6,#005cb8);color:#fff;cursor:pointer;font-size:13px;font-weight:600;font-family:'DM Sans',sans-serif;">
...
function showErrorModal(message, code) {
  ...
  overlay.innerHTML = `
    <div style="background:#fef2f2;border-left:3px solid #d4354f;border-radius:0 8px 8px 0;padding:14px 18px;max-width:480px;width:90%;box-shadow:var(--shadow-md);">
      ...
      <button onclick="this.parentElement.parentElement.remove()" style="margin-top:10px;padding:4px 14px;border:1px solid #fecaca;border-radius:6px;background:transparent;color:#b82940;cursor:pointer;font-size:11px;font-family:'DM Sans',sans-serif;">
        ${t('close') || 'Close'}
      </button>
    </div>`;
